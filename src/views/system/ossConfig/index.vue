...
import { pageOssConfig, removeOssConfigByIds } from '/@/api/resource/ossConfig';
...
const tableProps = shallowRef<BasicTableProps>({
  title: 'oss配置管理',
  getApi: (params) => pageOssConfig(params),
  toolbar: [
    {
      name: '新增',
    ...
    },
    {
      ...
      onClick: ({ selectedRowKeys }) => {
        deleteWarnModel().then(() => {
          removeOssConfigByIds(selectedRowKeys).then(() => {
            deleteSuccess();
